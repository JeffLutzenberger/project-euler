...
void product() {
    ...
    //seems like a tree problems....maybe?
    long max_prod = 0;
    for(int i = 0; i < 20; i++){
        for(int j = 0; j < 20; j++){
            //now get largest product in in the 4x4 matrix
            //need to check:
            //rows
            long product = 0;
            if(j < 20 - 4){
            ...
            if(i < 20 - 4 && j < 20 - 4){
                ...
                if(product > max_prod) max_prod = product;
                product = a[i+3][j] * a[i+2][j+1] * a[i+1][j+2] * a[i][j+3];
                if(product > max_prod) max_prod = product;

            }
        }
    }
    printf("max product: %li\n", max_prod);
}
